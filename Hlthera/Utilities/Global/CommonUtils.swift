...
let kAlertTitle = "Hlthera".localize
let kButtonTitle = "Ok".localize
let kNoInternetMsg = "Unable to connect to the Internet. Please try again.".localize
let kDefaultErrorMsg = "Something went wrong. Please try again.".localize
let kchangeMobileNumber = "changeMobileNumber".localize
...
class CommonUtils {
    ...
    static func showAlertForUnderDevelopment() {
        CommonUtils.showAlert(title: kAlertTitle,
                              message: "Under Development".localize,
                              buttonTitle: kButtonTitle) { (_) in
        }
    }
    ...
    static func showAlert(title: String,
        ...
        let alertController = UIAlertController(title: title.count>0 ? title: kAlertTitle,
                                                message: message.count > 0 ? message: "Message".localize,
    ...
    static func showAlertToPickImageOption(completion: @escaping(_ index: Int?) -> ()) -> Void {
        ...
        let alertController = UIAlertController(title: nil,
                                                message: "Please select an option to pick an image.".localize,
                                                preferredStyle: .actionSheet)
        
        let firstAction = UIAlertAction(title: "Camera".localize,
                                        style: .default,
                                        handler: { (alert) in
            completion(0)
        })
        
        let secondAction = UIAlertAction(title: "Gallery".localize,
                                         style: .default,
                                         handler: { (alert) in
            completion(1)
        })
        
        let thirdAction = UIAlertAction(title: "Cancel".localize,
                                        style: .cancel,
                                        handler: { (alert) in
            completion(2)
        })
    ...
    static func imagePickerCamera(viewController:UIViewController){
        ...
        if UIImagePickerController.isSourceTypeAvailable(.camera){
            ...
            FTIndicator.showToastMessage("Camera not found".localize)
    ...
    static func imagePicker(viewController:UIViewController){
        ...
        let actionSheet = UIAlertController(title: "Photo Source".localize, message: "Choose a source".localize, preferredStyle: .actionSheet)
        
        actionSheet.addAction(UIAlertAction(title: "Camera".localize, style: .default, handler: {(action: UIAlertAction) in
        ...
        actionSheet.addAction(UIAlertAction(title: "Photo Library".localize, style: .default, handler: {(action :UIAlertAction) in imagePicker.sourceType = .photoLibrary
        ...
        actionSheet.addAction(UIAlertAction(title: "Cancel".localize, style: .cancel, handler:
    ...
    class func saveFileToDocumentDirectory(url: URL?, fileName: String, completionHandler: @escaping (Bool?) -> ()){
        ...
        if url != nil {
            do {
                ...
                print_debug(items:"Unable to load data:".localize + "\(error)")
                ...
                if let urlData = NSData(contentsOf: videpUrl!){
                    ...
                    if let videoUrl = URL(string: filePath) {
                        ...
                        CommonUtils.saveVideoToAlbum(videoUrl) { (error) in
                            ...
                            if error == nil {
                                CommonUtils.showToast(message: "Video saved successfully".localize)
