...
//        self.Messageclass = MessageList.fetchMessagesForUser(userid: receiverid, bookingId: String.getString(self.communicationList?.appointmentID)) ?? []
...
//        let completeName = self.userDetails[Parameters.name] as? String ?? ""
//        
//        let dic = [Parameters.senderid: String.getString(self.userDetails[Parameters.user_id]), Parameters.content: String.getString(self.chatTextView.text) , Parameters.timeStamp : timeStamp , Parameters.receiverid  : String.getString(self.receiverid)  , Parameters.thumnilimageurl : "" , Parameters.mediatype : "text" ,  Parameters.isDeleted : "" , Parameters.mediaurl :  "" , Parameters.groupid :  "",Parameters.groupname : "","from_name" : completeName, "to_name" : self.receivername, Parameters.status : "not_seen"]
//
...
//            Chat_hepler.Shared_instance.ResentUser(lastmessage:  String.getString(self.chatTextView.text), Receiverid: String.getString(self.receiverid), Senderid: String.getString(self.userDetails[Parameters.user_id]), name: String.getString(self.receivername), profile_image: String.getString(self.receiverprofile_image), readState: "not_seen", isMsgSend: true, lastTimestamp: timeStamp, msgReceiverId: String.getString(self.receiverid), unreadCount: self.unread_count, friendStatus: self.isFriend)//String.getString(self.Messageclass.count)
...
//    func ReceiveMessage() {
//        if self.CreatedBy == Parameters.emptyString {
//            Chat_hepler.Shared_instance.Receivce_message(Senderid: String.getString(self.userDetails[Parameters.user_id]), Receiverid: String.getString(self.receiverid), bookingId: String.getString(self.communicationList?.appointmentID)) {[weak self] (value, ChatBackUp, success, isExist)  in
//                if success {
...
//            
//            Chat_hepler.Shared_instance.ResentUser(lastmessage:  String.getString(self.Messageclass.last?.Message), Receiverid: String.getString(self.receiverid), Senderid: msgSenser_id, name: String.getString(self.receivername), profile_image: String.getString(self.receiverprofile_image), readState: String.getString(self.Messageclass.last?.status), isMsgSend: false, lastTimestamp: String.getString(self.Messageclass.last?.SendingTime), msgReceiverId: msgReceiver_id, unreadCount: self.unread_count, friendStatus: self.isFriend)
//            }
//            
//                self.filterMessageArray()
//                self.chatTblView.reloadData()
